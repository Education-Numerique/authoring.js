(function () {
    ...
    this.RedactorPlugins.perf = new (function () {
        this.init = function () {
            var redactorScope;
            var target = null;
            var htmlDisplayedConditionaly;

            var callback = (function () {

                if (target) {
                    htmlDisplayedConditionaly = target;
                    $('#redactor_modal .condition').val(target.attr('data-condition'));
                    $('#redactor_modal .redactor_btn_modal_remove').show();
                } else {
                    this.saveSelection();
                    htmlDisplayedConditionaly = this.getSelectedHtml();
            ...
            this.addBtn('Perf', 'Affichage conditionel', function (obj/*, e*/) {
                redactorScope = obj;
            ...
            var insertFromMyModal = function (el) {
                ...
                if (conditionSyntax.test(trimmedConditon)){
                    var markup = $('<div><a data-type="perf" />&nbsp</div>');
                    markup.find('a').html(htmlDisplayedConditionaly);
                    markup.find('a').attr('data-condition', redactorScope.stripTags(condition));
                    markup.find('a').attr('title', redactorScope.stripTags(condition));

                    if (el && el.attr('data-type') == 'perf') {
                    ...
                    $('#redactor_modal .error_condition').text('Erreur de syntaxe');
                }
            };
        };
