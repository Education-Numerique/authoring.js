...
(function () {
    ...
    LxxlLib.Em.Wysiwyg = Em.TextArea.extend({
        ...
        didInsertElement: function () {
            ...
            // BEGIN of "Perf" plugin
            Object.getPrototypeOf(api).observesPerf = function () {
                var showPerfButton = (function (e) {
                    insertPerfButton();
                    this.$editor.perfButton.hide();

                    if (!this.getSelectedHtml().trim())
                        return;


                    var width = this.$editor.perfButton.innerWidth();
                    var left = e.clientX;
                    ...
                    this.$editor.perfButton.css({ left: left + 'px', top: (e.clientY + $(document).scrollTop() +
                        14) + 'px' }).show();
                    return true;
                }.bind(this));

                var insertPerfButton = (function () {
                    if (!this.$editor.perfButton) {
                        this.$editor.perfButton = $('<div class="redactor_air redactor_perf_air" style="">' +
                            '<ul class="redactor_toolbar">' +
                            '<li>' +
                            '<a href="javascript:void(null);" title="Affichage conditionel" class="redactor_btn_perf"></a>' +
                            '</li>' +
                            '</ul>' +
                            '</div>');
                        $('body').prepend(this.$editor.perfButton);
                        this.$editor.perfButton.hide();

                        this.$editor.perfButton.find('a').click(function (/*e*/) {
                            api.getBtn('perf').click();
                            this.$editor.perfButton.hide();
                        }.bind(this));
                    }
                }.bind(this));

                this.$editor.bind('textselect', showPerfButton.bind(this));
            ...
            if (plugins.indexOf('perf') != -1) {
                $(editor).on('click.perf-click-handler', '[data-type=perf]', function (e) {
                    api.getBtn('perf').data('target', e.target);
                    api.getBtn('perf').click();
                });
                api.observesPerf();
